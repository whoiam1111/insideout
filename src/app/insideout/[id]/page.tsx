...
export default function MoimDetailPage({ params }: MoimDetailPageProps) {
	...
	const { id } = React.use(params); // ✅ 이렇게 언래핑해야 함
	...
	return (
		<>
			<div className="py-6 lg:pb-10 bg-white min-h-screen">
				<section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col">
					{moim.imageUrl && (
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-gray-700 text-sm sm:text-base">
						{moim.address && (
							<p>
								<strong className="text-gray-900">장소:</strong>{" "}
								{moim.address}
							</p>
						)}
						{moim.category && (
						...
						{moim.city && (
							<p>
								<strong className="text-gray-900">지역:</strong>{" "}
								{moim.city} {moim.district}
							</p>
						)}
						{moim.capacity !== undefined && (
							<p>
								<strong className="text-gray-900">정원:</strong>{" "}
								{moim.capacity}명
							</p>
						)}
						{moim.duration_type && (
						...
						{moim.time && (
							<p>
								<strong className="text-gray-900">시간:</strong>{" "}
								{moim.time}
							</p>
						)}
						{moim.date && (
							<p>
								<strong className="text-gray-900">일정:</strong>{" "}
								{moim.date}
							</p>
						)}
						{moim.start_date && moim.end_date && (
							<p>
								<strong className="text-gray-900">기간:</strong>{" "}
								{moim.start_date} ~ {moim.end_date}
							</p>
						)}
						{moim.address && (
							<p>
								<strong className="text-gray-900">주소:</strong>{" "}
								{moim.address}
							</p>
						)}
					</div>
					...
					<Link
						href={""}
						className="bg-indigo-500 text-white w-full text-center rounded-lg py-4 hover:bg-indigo-600
                            text-lg md:text-xl"
					>
						신청하기
					</Link>
